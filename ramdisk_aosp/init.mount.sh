...
if [ ! -f /system/priv-app/SystemUI/SystemUI.apk ] ; then
	export ext4=0
	umount -f /system
	mount -t ext4 -o noatime,nodiratime,nosuid,nodev,barrier=1,data=ordered,nomblk_io_submit,errors=panic /dev/block/platform/msm_sdcc.1/by-name/userdata /arter97/data
	...
	rm -rf /arter97
	mount -t ext4 -o noatime,nodiratime,nosuid,nodev,barrier=1,data=ordered,nomblk_io_submit,errors=panic /dev/block/platform/msm_sdcc.1/by-name/userdata /data || export ext4=0
	...
	mount -t ext4 -o noatime,nodiratime,nosuid,nodev,barrier=1,data=ordered,nomblk_io_submit,errors=panic /dev/block/platform/msm_sdcc.1/by-name/cache /cache
...
if [[ $ext4 == "1" ]]; then
	sed -i -e 's/#EXT4/\/dev\/block\/platform\/msm_sdcc.1\/by-name\/userdata     \/data           ext4    noatime,nosuid,nodev,barrier=1,data=ordered,nomblk_io_submit,errors=panic    wait,check,encryptable=\/dev\/block\/platform\/msm_sdcc.1\/by-name\/metadata/g' /fstab.flo
	umount -f /data
fi
